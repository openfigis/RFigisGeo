...
readWFS <- function(url, outputFormat = "GML", p4s = NULL,
  ...
  if(outputFormat == "GML") {
    ...
    #escaping characters in cql_filter
    percentCodes <- c("%20", "%27")
    names(percentCodes) <- c("\x20", "\x27")
    if(grepl("cql_filter", wfsRequest)){
      arr <- unlist(strsplit(wfsRequest, "&cql_filter="))
      wfsBaseRequest <- arr[1]
      cql_filter <- arr[2]
      otherParams <- NULL
      arr_suffix <- unlist(strsplit(cql_filter, "&"))
      if(length(arr_suffix)>1){
        cql_filter <- arr_suffix[1]
        otherParams <- arr_suffix[2:length(arr_suffix)]
      }
      cql_filter <- curlPercentEncode(x = curlEscape(cql_filter), codes = percentCodes)
      wfsRequest <- paste0(wfsBaseRequest, "&cql_filter=", cql_filter)
      if(!is.null(otherParams)){
        wfsRequest <- paste0(wfsRequest, "&", paste(otherParams, collapse="&"))
      }
    }
    
    content <- getURL(wfsRequest, .encoding = "UTF-8")
